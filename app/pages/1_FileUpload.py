...
# --- 4. 디버깅용 (개발 중에만 보이고 나중엔 지우세요) ---
st.divider()
st.subheader("🔍 현재 저장된 데이터 상태 (디버깅용)")
st.json({
    "API Key 저장 여부": "O" if st.session_state['user_api_key'] else "X",
    "데이터 타입": st.session_state['content_type'],
    "데이터 내용(일부)": str(st.session_state['uploaded_content'])[:50] + "..." if st.session_state['uploaded_content'] else "없음"
})
